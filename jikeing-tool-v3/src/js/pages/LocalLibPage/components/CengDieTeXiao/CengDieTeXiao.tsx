...
import styles from './CengDieTeXiao.module.scss'

interface IProps {

}

const CengDieTeXiao: React.FC<IProps> = (props) => {
    ...
    const reloadFolderList = () => {
        evalTS("getFolderContent", "overlay", "mp4", "mov").then(content => {
    ...
    function fileRef2VideoSrc(fileRef: FileRef): any {
        return `file:///${fileRef.path}`
    }
    ...
    function showFileList(path: string) {
        ...
        //       effectList.value = response;
        //     })
        //   }
        //   else if (funSelected.value == '叠层') {
        evalTS("getVideoFileList", path).then(response => {
            // 过滤掉
            setFileList(response.filter(c => c.isFileOrFolder == true))
        })
        //   }
