import styles from './LocalAudioList.module.scss'
...
const LocalAudioList: React.FC<IProps> = (props) => {
    ...
    var audioPlayer: Howl | null = null

    const reloadFolderList = () => {
        evalTS("getFolderContent", "audio", "mp3", "aac").then(content => {
    ...
    // update the playing audio path
    const updatePlayingAudioPath = (path: string | null) => {
        if (path) {
            var sound = new Howl({
                src: [path],
                html5: true
    ...
    return <>
        <div className="flex text-[14px] p-[16px]" style={{ height: 'calc(100% - 15px)' }}>
            ...
            <div className="flex w-full h-full" >
                <div className={styles.audioGridContainer}>
                    {
                        fileList.map((f, index) => <div key={f.name}
                            className={styles.audioGridItem}
                            onClick={() => evalTS("addResource", f.path)}
                            onMouseEnter={() => updatePlayingAudioPath(f.path)}
                            onMouseLeave={() => updatePlayingAudioPath(null)}
                        >
